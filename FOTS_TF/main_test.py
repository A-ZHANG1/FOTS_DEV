...
def get_project_matrix_and_width(text_polyses, target_height=8.0):
    ...
    for i in range(text_polyses.shape[0]):
        ...
        #rotated_rect = cv2.minAreaRect(cnt)#报错的原因是 cnt 是float64 类型，cnt只能是float32或者int32
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for im_fn in im_fn_list:
                ...
                if boxes is not None and boxes.shape[0] != 0:
                    res_file_path = os.path.join(FLAGS.output_dir,'rec/', 'res_' + '{}.txt'.format(os.path.basename(im_fn).split('.')[0]))
                    res_file_path_2 = os.path.join(FLAGS.output_dir,'det/', 'res_' + '{}.txt'.format(os.path.basename(im_fn).split('.')[0]))
                    ...
                    with open(res_file_path_2, 'w') as f:
                        for i, box in enumerate(boxes):
